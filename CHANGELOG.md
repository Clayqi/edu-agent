...
### 变更
- **教案中心页面整页下线**：导航项 `design` 移除、`#view-design` 与「教案 × PPT 编排台」删除、
  「对话内联导出条」移除。看稿与下载统一由右侧预览面板承担。
- **模板工作台整体移除**（按维护者要求「不要模板，保留导入」）：导入 / 载入编辑 / 新建空白 / 保存 /
  删除 / 格式工具栏 / 按模板导出 全部下掉。**后端 `/api/template/*` 一行未动**，
  `content/templates/` 里的文件也没删；①「生成教案模板」与②「按模板生成」仍可用，
  只是生成出来的模板无法在界面里编辑或删除（要清理请直接操作 `content/templates/`）。
