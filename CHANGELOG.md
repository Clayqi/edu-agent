...
- **「边框里的 PPT 优化」（一期）**：在右侧侧边面板的「文档」视图 →【幻灯片】页签里
  **读入一份已有的 .pptx → 体检 → 保守原地统一 → 内容零丢失校验 → 下载**
  （详见 `docs/20-PPT优化.md`，计划与五个候选 skill 的判定见 `docs/19-边框里的PPT优化计划书.md`）：
...
- **一期不做"整套重排/美化"**：ppt-master 的 `beautify-pptx` 档是**模型驱动的 SVG 重排**
  ...
  一期只吃它的确定性部分，真重排排到二期（`docs/19` §4.1.1 有完整判定）。
...
### 新增
- **预览文档可以直接编辑**（老师：要的是"可以进行编辑的预览文档"）：
  - **文档页签所见即所得**：正文可直接改，上面一条工具栏（加粗 / 斜体 / 大标题 / 小标题 / 项目符号 / 编号 / 清格式）；
  - **Markdown 页签改成可编辑源码框**：改 Markdown 也能实时回流；
  - 改动 400ms 防抖后回流到 `/api/session/doc-preview`（**纯内存**）重算板块 / 幻灯片 / 统计 / 标题，
    正在编辑的页签不重渲染（光标不跳）；**导出的 Word / PPT / HTML 用的就是改后的内容**；
  - 改动镜像到 `localStorage`（键带会话与标题，且只在原文未变时恢复），**刷新/收起再展开都不丢**；没改就不写。
- **每条回答都能「生成文档」**：AI 回答下方的操作行从【复制】【重新生成】扩成三个按钮，
  第三个 **「生成文档」** 把这条回答（**含答疑**）送进右侧预览面板
  （`POST /api/session/doc-preview`，纯内存）；同一条再点一次**直接打开、不重复请求**。
  这是会话页 → 预览面板的**唯一入口**。
- **Agent 出稿自动填充面板**：`done` 事件新增 `doc` 结构化 payload（`_doc_payload()` 纯计算：
  标题 / markdown / 板块 / 幻灯片页 / 统计），前端收到即填充并打开面板；
  同一份 payload 也写进会话 `meta.doc`，**刷新/切会话后从历史重载**（`docpArm()`，只备好不弹窗）。
- **4 个 Tab 换成真正的库渲染**（面板外壳一行未动，只替换 Tab 内部渲染逻辑）：
  - **Markdown = marked + DOMPurify** 渲染的富文本（原来的纯文本 `<pre>` 退休）；
  - **文档 = 类 Word 排版**（标题层级 / 列表 / 表格边框 / 段距），导入 `.docx` 走 mammoth；
  - **幻灯片 = pptxgenjs 内存建模** + 16:9 分页卡片（**不调用 `writeFile`，不产生磁盘文件**）；
  - **结构 = 板块清单 + 可展开的「原始 JSON」**（`<details>`）。
- **依赖加载双保险**（本轮实测踩到）：4 个 CDN 标签加 **`defer`** 不再阻塞首屏；
  新增 `dpEnsureLibs()`：渲染前检查全局库，缺哪个就自己插一个本地 `static/vendor/` 脚本，
  补齐后回调**重渲染一次**（最多补 2 轮）。断网/被墙时 4 个库仍全部就位，库彻底拿不到也不白屏。

### 变更
- **侧边面板上面那排按钮改成「紧凑一排 + 可收起」，文档区变大**（老师：五个按钮太大、显示文档的地方太小）：
  - 五个圆钮原本是 2 列 76px 大圆（5 个要占 ~320px 高），**文档视图里自动收成一行紧凑按钮**
    （高 28px、图标+文字横排），且**默认收起**，把高度整个让给文档；
  - 面板右上角新增 **「收起 / 展开」** 按钮，随手可切（状态记 `localStorage: edu_sp_quad_collapsed`，
    手动选过就按手动选的，不选则文档视图自动收起）；收起时留一行提示；
  - **文档面板默认宽度 460 → 680px**（上限放宽到 `min(1000px, 80vw)`，双击复位也回到 680）；
    文档视图里面板内边距从 `18px 16px` 收到 `12px 14px 10px`、`#spBody` 上边距 30px → 6px。
- **文档预览真正并进应用自带的侧边面板**（老师反馈"查看文档与文档的框根本没有融入侧边面板"）——
  查下去是**结构放错了**：应用本来就有一块 `aside#sidePanel`（右侧 300px，标题「侧边面板」，
  内含 状态 / 引用 / 知识源 / 会话），而我把预览做成了另一块 `#docPanel`，只能靠 `position:fixed` 硬浮，
  于是右边同时有两块东西。现在：
  - **独立的 `#docPanel` 整个删掉**（元素 / 样式 / 悬浮按钮 / 收起竖条），预览成为 `#sidePanel` 的
    **第 5 个视图**「文档」（圆钮带角标），标题行（导入 / 下载▾ / 位置）+ 四个页签 + 编辑工具栏 +
    正文 + 结果行都渲染进面板的 `#spBody`；
  - **文档视图下这块面板自动加宽**（320 ~ 70vw，默认 560px），左边缘可拖拽调宽，宽度仍记忆在 `--docp-w`；
  - 整块面板的开合交回标题栏原有的 **「侧边面板」** 按钮；编辑功能与数据源一行没改；
  - 结果：`.shell` 的右侧**只剩一块面板**，不再有两个右边框。
- **入口与预览窗口全部融进侧边面板，右侧只剩一条框**（原来面板外还挂着一个悬浮按钮/把手，展开时
  和面板并排成两条竖框）：
  - 面板**收起态就是它自己缩成的那条竖把手**（`#docPanel.dp-collapsed`，宽 30px 贴右缘，
    文案「导入文档 / 查看文档」），点它展开；`#docPanelFab` 元素与样式**整个删掉**；
  - 收起不再 `display:none`，展开后入口都在标题栏（【导入】【下载▾】【位置】【×】）；
  - **幻灯片的 16:9 预览搬进面板**（顶部预览台 + 上下页 + 页卡跳页），去掉「在新窗口看」；
    教案中心编排台的 **「预览 PPT」也送进同一个面板**，不再弹新窗口。
- **会话里去掉「下一步」计划卡片**（应老师要求：每条回答都弹一张卡片太吵）：
  标题行「下一步（教案 Agent）」、①②③ 按钮、提示行**都不再渲染**，会话侧只留操作行的
  【复制】【重新生成】【生成文档】。实现是前端一个开关 `PLAN_CARD_ON = false`（后端 `options` 事件
  照旧下发、接口一行未改，改回 `true` 即恢复）；设置弹窗里那行 **「教案选项自动提示」** 一并撤掉
  （卡片都不渲染了，开关留着只会骗人）。
  **代价（明确记录）**：①生成教案模板 / ②按模板生成 / ③按模板填入 在会话里没有入口了，
  教案模板相关操作改从**教案中心**走；后端 `/api/plan/*` 与 `tests/test_plan_link.py` 全部保留。
- **「查看/导入文档」入口改成贴面板边缘的竖排把手**（原来在屏幕右下角悬浮，与面板是两块分开的东西）：
  面板收起时贴在屏幕右缘，展开后贴到**面板左边缘**，文字三态不变（导入文档 / 查看文档 / 收起文档）；
  宽度复用面板的 `--docp-w` 变量，**拖宽面板时把手自动跟随**。面板内部（Tab、下载▾、收起、拖拽）一行未动。
- **修掉面板把 Agent 标签当正文的问题**：会话库里存的回答正文带 `_tag()` 加的
  `**【Agent B · 教案 Agent】**`，历史兜底进面板时标题因此退化成「未命名文档」、标签还会渲染进
  四个 Tab 与导出文件。现在按"行首前缀"摘掉（B/A 的标签**与正文粘在同一行**，整行匹配会漏），
  已落库的 `meta.doc` 在打开会话时一并清洗；标题优先级改为 payload 标题 > 正文首个标题 > 占位。
- **会话侧不再有"跳去教案中心"的按钮**：计划卡片 ③ 的文案由「填入教案中心」改为
  **「按模板填入」**（后端 label 同步改），提示改为"结果直接进右侧预览面板"；
  回答尾部那条重复的「在右侧预览里查看」链接删除（同一个动作不再有两个入口）。
- 面板提示小字定为 **「右侧为内存预览效果，实际排版以导出 WPS 文件为准」**。
- **教案中心 / 编排台 / 导入 `.docx` 全部保留**（上一版只下掉了模板编辑器，本次未再改动）。

### 兼容性
- **后端 WPS 链路一行未改**：`src/edu_agent/wps_export.py` 零改动；`/api/wps/export`、
  `/api/wps/download`、WPS MCP 白名单、`deploy/*` 原样；预览链路**不查 WPS 开关、不调 MCP、不落盘**。
- 实时链路实测（`preset=auto`，「帮我备一节 3.3 幂函数的课」）：
  `session → options(计划卡片) → done`，`mode=B`、`html=3.3_幂函数.html`、`done.doc` 就位，
  `meta.doc` 落库可回放。
- 附件：`tests/test_wps_tools.py` **34 例**（含 `TestSessionDocPreview` 9 例：payload 透传、
  skill 开关、MCP 关闭、零磁盘/零 MCP 调用、Agent 标签剥离与历史 `meta.doc` 清洗）全过；
  jsdom 真 DOM 回归 **5 组场景全绿**
  （①每条回答都有入口 ②四 Tab 各自渲染器 + 导出 ③库缺失降级 ④计划卡片入口整合
  ⑤skill 关闭 / WPS 关闭），零运行时错误；另加一条真数据探针：旧会话点把手 → 面板标题取到真标题、
  四 Tab 无标签、把手位置随面板宽度变化。

## [v1.13.0] - 2026-09-12

### 新增
- **会话内文档预览面板**：AI 出文档后，**会话页右侧自动出现可拖拽的多格式预览面板**
  （详见 `docs/14-会话内文档预览.md`，设计与取舍见 `docs/15-会话内预览改造计划书.md`）：
  - **四个页签**：文档（排版渲染）/ Markdown（原文）/ 幻灯片（每环节一页 + 在新窗口看 16:9）/ 结构（板块 + 字数）；
  - **可拖拽调宽**：拖左边缘 320 ~ min(920,74vw)，双击复位 460，宽度记忆在 `localStorage: edu_docpanel_w`；
    面板打开时会话内容自动让位；
  - **下载合并成一个按钮**：原 Word/PPT/HTML 三个按钮合并为 `下载 ▾`，**鼠标悬停弹出下载方式菜单**
    （点击也能开，兼容触屏；选完自动收起）；WPS 未就绪时落盘两项灰显但仍可点（点了说明原因），HTML 项始终可用；
  - 历史恢复的教案回答**不自动弹面板**（避免盖住会话），最近一份备好待用。
- **导入 .docx 进预览**：面板标题栏「导入」→ 服务端解析成富结构 → **只在面板里看与下载**
  （逐 run 保格式、含真表格；下载走 `template:` 原格式直出）。**不写模板库。**
- 新 skill 能力开关 **`doc-session-preview`**（「会话文档预览」，默认开启，内置能力不绑 MCP）：
  关掉只让会话页不渲染面板，**导出链路完全不受影响**。Skill 市场可开关，详情显示「内置」。
- 新接口 **`POST /api/session/doc-preview`**：**纯内存**（不落盘、不调用 WPS MCP、不查 WPS 开关），
  payload/markdown 原样回传 + 派生预览模型（标题 / 板块 / 幻灯片 / 统计）。
- `GET /api/mcp/status` 新增字段 **`session_doc_preview_ready`**（`/api/capabilities` 快照同样带）。

### 变更
- **教案中心只移除「模板编辑器」**：页面、导航与**教案 × PPT 编排台原样保留**；
  模板行现在只剩 **「导入 .docx」**（解析后只进右侧预览，不写模板库），
  「载入编辑 / 新建空白 / 删除 / 保存为模板 / 退出模板模式 / 格式工具栏 / 编辑器」全部下掉。
  「对话内联导出条」移除，看稿与下载统一由右侧预览面板 + 右下角常驻入口承担。
- **右下角常驻入口**（会话页与教案中心都可见）：没有文档时是「导入文档」（直接开文件选择），
  有文档时是「查看文档 / 收起文档」——解决「关掉面板或刷新页面后就找不回来」。
- **预览面板移到全局层**（不再嵌在会话页内），所以在教案中心导入也能就地看到结果；
  打开时 `body` 加 `.dp-open`，会话页消息区与教案中心同时让出右侧空间。
- 「③ 填入」的结果改为**直接进右侧预览面板**（不再进模板编辑器）。
- **后端 `/api/template/*` 一行未动**，`content/templates/` 里的文件也没删；
  ①「生成教案模板」与②「按模板生成」仍可用，只是生成出来的模板无法在界面里编辑或删除
  （要清理请直接操作 `content/templates/`）。

### 修复
- **`static/index.html` 的「整文件 diff」陷阱**（本次踩到）：该文件在 git 索引里被标成 `-text`（binary），
  git 于是**跳过它的行尾归一**——只要动一行，`git diff` 就把整个文件 3200+ 行都报成改动，评审没法看。
  已把工作区统一为 LF、索引恢复 `i/lf w/lf`，并新增 **`.gitattributes`**（`* text=auto` + 逐类型点名文本 +
  `.cmd/.bat` 强制 CRLF）根治，避免复发。
- 面板唤起不再先等 `capLoad()`：改以**接口返回的 `enabled`** 为准（服务端唯一真相源），
  否则首屏那次 `/api/capabilities` 的 WPS 桥接探测会把面板出现拖慢好几秒。

### 兼容性
- **后端 WPS 链路一行未改**：`wps_export.py` 零改动；`/api/wps/export`、`/api/wps/download` 行为不变；
  `mcp_tools.py`、`deploy/*`、wps-skills 安装脚本、`.gitignore` 护栏原样。
- 附件：`tests/test_wps_tools.py` 32 例（新增 `TestSessionDocPreview` 7 例，含**双向解耦**与
  **纯函数校验**：预览调用前后导出目录文件集合不变、零 MCP 调用）；全量 **215 例通过**。
- jsdom 真 DOM 回归扩到**六个场景**，全过、零运行时错误；其中场景 F 用**真 .docx** 走完整导入链路
  （面板渲染出真表格、下载请求体走 `template:` 分 12 板块）。
